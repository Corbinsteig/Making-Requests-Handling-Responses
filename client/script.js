...
const repeatMyParam = () => {
    return axios.get(`http://localhost:3000/repeat/Tiger`)
    .then((response) => {
    const repeatEl = document.getElementById('repeat-text');
...
const getRequest = (event) => {
    event.preventDefault()
    axios.get('http://localhost:3000/query-test/?name=yolo&job=sleep')
    .then((res) => {
        console.log(res.data)
    })
    .catch(err => {
...
const createFood = (event) =>{
    ...
    document.querySelector('#food-button').addEventListener('submit', createFood)